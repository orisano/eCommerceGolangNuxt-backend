...
type AttributeQuery struct {
	...
	// eager-loading edges.
	withAttribute *SellerProductVariationValuesQuery
...
func (aq *AttributeQuery) QueryAttribute() *SellerProductVariationValuesQuery {
	query := &SellerProductVariationValuesQuery{config: aq.config}
...
func (aq *AttributeQuery) WithAttribute(opts ...func(*SellerProductVariationValuesQuery)) *AttributeQuery {
	query := &SellerProductVariationValuesQuery{config: aq.config}
...
func (aq *AttributeQuery) sqlAll(ctx context.Context) ([]*Attribute, error) {
	...
	if query := aq.withAttribute; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Attribute = []*SellerProductVariationValues{}
		}
		query.withFKs = true
		query.Where(predicate.SellerProductVariationValues(func(s *sql.Selector) {
			s.Where(sql.InValues(attribute.AttributeColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.attribute_attribute
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "attribute_attribute" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "attribute_attribute" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Attribute = append(node.Edges.Attribute, n)
		}
	}
