...
type CategoryQuery struct {
	...
	withProductCategories *SellerProductCategoryQuery
...
func (cq *CategoryQuery) QueryShopCategory() *ShopCategoryQuery {
	query := &ShopCategoryQuery{config: cq.config}
...
func (cq *CategoryQuery) QueryProductCategories() *SellerProductCategoryQuery {
	query := &SellerProductCategoryQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CategoryQuery) QuerySellerProducts() *SellerProductQuery {
	query := &SellerProductQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CategoryQuery) WithShopCategory(opts ...func(*ShopCategoryQuery)) *CategoryQuery {
	query := &ShopCategoryQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withShopCategory = query
	return cq
}
...
func (cq *CategoryQuery) WithProductCategories(opts ...func(*SellerProductCategoryQuery)) *CategoryQuery {
	query := &SellerProductCategoryQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CategoryQuery) WithSellerProducts(opts ...func(*SellerProductQuery)) *CategoryQuery {
	query := &SellerProductQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withSellerProducts = query
	return cq
}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context) ([]*Category, error) {
	var (
		...
		_spec       = cq.querySpec()
		loadedTypes = [5]bool{
			cq.withShopCategory != nil,
			cq.withParent != nil,
			cq.withChildren != nil,
			cq.withProductCategories != nil,
			cq.withSellerProducts != nil,
		}
	)
	if cq.withShopCategory != nil || cq.withParent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withShopCategory; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Category)
		for i := range nodes {
			if nodes[i].shop_category_categories == nil {
				continue
			}
			fk := *nodes[i].shop_category_categories
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "shop_category_categories" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := cq.withProductCategories; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ProductCategories = []*SellerProductCategory{}
		}
		query.withFKs = true
		query.Where(predicate.SellerProductCategory(func(s *sql.Selector) {
			s.Where(sql.InValues(category.ProductCategoriesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.category_product_categories
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "category_product_categories" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "category_product_categories" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ProductCategories = append(node.Edges.ProductCategories, n)
		}
	}

	if query := cq.withSellerProducts; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Category, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.SellerProducts = []*SellerProduct{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(category.SellerProductsPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, cq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "seller_products": %w`, err)
		}
		query.Where(sellerproduct.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "seller_products" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.SellerProducts = append(nodes[i].Edges.SellerProducts, n)
			}
		}
	}
