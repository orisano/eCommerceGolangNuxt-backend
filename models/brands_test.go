...
func testBrandToManySellerProducts(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.BrandID, a.ID)
	queries.Assign(&c.BrandID, a.ID)
	...
	for _, v := range check {
		if queries.Equal(v.BrandID, b.BrandID) {
			bFound = true
		}
		if queries.Equal(v.BrandID, c.BrandID) {
			cFound = true
		}
	}
	...
	slice := BrandSlice{&a}
	if err = a.L.LoadSellerProducts(ctx, tx, false, (*[]*Brand)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testBrandToManyAddOpSellerProducts(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if !queries.Equal(a.ID, first.BrandID) {
			t.Error("foreign key was wrong value", a.ID, first.BrandID)
		}
		if !queries.Equal(a.ID, second.BrandID) {
...
func testBrandToManySetOpSellerProducts(t *testing.T) {
	var err error
	...
	if !queries.IsValuerNil(b.BrandID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.BrandID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.BrandID) {
		t.Error("foreign key was wrong value", a.ID, d.BrandID)
	}
	if !queries.Equal(a.ID, e.BrandID) {
		t.Error("foreign key was wrong value", a.ID, e.BrandID)
	}
...
func testBrandToManyRemoveOpSellerProducts(t *testing.T) {
	var err error
...
var (
	brandDBTypes = map[string]string{`ID`: `bigint`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`, `Name`: `text`}
	_            = bytes.MinRead
)
