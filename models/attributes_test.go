...
func testAttributeToManySellerProductVariationValues(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadSellerProductVariationValues(ctx, tx, false, (*[]*Attribute)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testAttributeToManyAddOpSellerProductVariationValues(t *testing.T) {
	var err error
...
var (
	attributeDBTypes = map[string]string{`ID`: `bigint`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`, `Name`: `text`}
	_                = bytes.MinRead
)
